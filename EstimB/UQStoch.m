...
for Step = 1: NoSteps % && error_u_L2_1 > 0.04*10^(-2)/1
    ...
    b_w_star = sqrt(1-2*delta)*b_w_n + sqrt(2*delta)*xi_k;
    ...
    Phi_n = PhiFunction(tau_w_exact,b_w_n,z_esc,Tmean_exact,B,qv0,qvs0,theta0,g,epsbar,LH,Qt_parcel0,Thetae_parcel0,Nt,z0,w0,dt,dW,Ns);
    
    Phi_star = PhiFunction(tau_w_exact,b_w_star,z_esc,Tmean_exact,B,qv0,qvs0,theta0,g,epsbar,LH,Qt_parcel0,Thetae_parcel0,Nt,z0,w0,dt,dW,Ns);

    alpha_b_w_star_n = min(1,exp(Phi_n-Phi_star));
    
    t = rand;
    
    if alpha_b_w_star_n >= t && b_w_star > 0
        b_w_n = b_w_star;
        ...
        figure(1)
        hold on
        plot(iter,b_w_avg_plot,'r *')
        plot(iter,b_w_min,'r o')
        plot(iter,b_w_n,'r +')
        plot(iter,b_w_exact,'b +')
        %plot(x(3:N+1,1),B_n(3:N+1,1),'b -',x,B_exact,'r -')
        title(num2str(Phi_n))
        legend('b_w avg','b_w min','b_w_n','b_w exact')
