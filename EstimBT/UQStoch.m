...
beta1 = .2; %2; %10; %Explorar mas
beta2 = .4; %2; %10; %Explorar mas
delta1 = 0.15; 
delta2 = 0.15; 
...
for Step = 1: NoSteps % && error_u_L2_1 > 0.04*10^(-2)/1
    ...
    tau_w_star = sqrt(1-2*delta1)*tau_w_n + sqrt(2*delta1)*xi_k_1;
    ...
    b_w_star = sqrt(1-2*delta2)*b_w_n + sqrt(2*delta2)*xi_k_2;
    ...
    Phi_n = PhiFunction(tau_w_n,b_w_n,z_esc,Tmean_exact,B,qv0,qvs0,theta0,g,epsbar,LH,Qt_parcel0,Thetae_parcel0,Nt,z0,w0,dt,dW,Ns);
    
    Phi_star = PhiFunction(tau_w_star,b_w_star,z_esc,Tmean_exact,B,qv0,qvs0,theta0,g,epsbar,LH,Qt_parcel0,Thetae_parcel0,Nt,z0,w0,dt,dW,Ns);
...
%         figure(1)
%         hold on
%         plot(tau_w_avg_plot,b_w_avg_plot,'r *')
%         plot(tau_w_min,b_w_min,'r o')
%         plot(tau_w_n,b_w_n,'r +')
%         plot(tau_w_exact,b_w_exact,'b +')
%         %plot(x(3:N+1,1),B_n(3:N+1,1),'b -',x,B_exact,'r -')
%         title(num2str(Phi_n))
%         legend('avg','min','n','exact')
%         print('test','-depsc',figure(1))
        ...
        bWAvgArray(iter,1) = b_w_avg_plot;
        bWminArray(iter,1) = b_w_min;
        bWnArray(iter,1) = b_w_n;
...
save('Resultados.mat','TauWAvgArray','TauWminArray','TauWnArray','bWAvgArray','bWminArray','bWnArray','Phi_nArray','Phi_minArray')
